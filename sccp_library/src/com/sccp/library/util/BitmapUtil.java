...
public class BitmapUtil {
     ...
     * @return 压缩后的Bitmap
     */
    public static ByteArrayOutputStream compressImage(Bitmap bitmap, int quality, CompressFormat cf) {
    	...
    	if(quality == 100) {
    		...
    		bitmap.compress(cf, quality, byteArrayOutputStream);// 质量压缩方法，这里100表示不压缩，把压缩后的数据存放到baos中
            
    		Log.d(TAG + "compressImage", String.valueOf(byteArrayOutputStream.toByteArray().length / 1024));
            while (byteArrayOutputStream.toByteArray().length / 1024 > 400) { //循环判断如果压缩后图片是否大于100kb,大于继续压缩
            	
            	byteArrayOutputStream.reset();
                bitmap.compress(cf, quality, byteArrayOutputStream);//这里压缩options%，把压缩后的数据存放到baos中
                quality -= 10;// 每次都减少10
            }
    	}
    	else{
    		bitmap.compress(cf, quality, byteArrayOutputStream);
    	}
	 ...
	 * @return
	 */
	public static Bitmap fileToCompressBitmap(String filePath, int width, int height) {
     ...
     * @return 期望Bitmap
     */
    public static Bitmap getRoundedCornerBitmap(Bitmap bitmap, float roundPx, boolean isCenter) {
        ...
        Log.d("getRoundedCornerBitmap", "Left="+ left +", Top="+ top +", Right="+ right +", Bottom="+ bottom);
